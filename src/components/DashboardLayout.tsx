    ...
    <div className="flex h-screen overflow-hidden bg-background">
      <aside className="hidden md:flex w-56 flex-col border-r border-border bg-sidebar">
        ...
        <nav className="flex-1 p-3 space-y-0.5">
          {navItems.map(item => {
            ...
            return (
              ...
              >
                <span className="font-mono text-[10px] text-muted-foreground group-hover:text-primary transition-colors">{item.num}</span>
                <item.icon className="w-3.5 h-3.5" />
                <span className="font-medium text-xs tracking-wide">{item.label}</span>
                {active && <motion.div layoutId="activeNav" className="ml-auto w-1 h-4 rounded-full bg-primary" />}
              </Link>
            );
          })}
        </nav>
        <div className="p-4 border-t border-border space-y-2">
          ...
          <div className="flex items-center gap-2">
            <span className={cn("w-1.5 h-1.5 rounded-full animate-pulse-dot", loading ? "bg-warning" : "bg-success")} />
        ...
        <header className="border-b border-border bg-card/60 backdrop-blur-md px-4 md:px-5 py-2.5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1 bg-secondary/80 rounded-lg p-1 border border-border">
              {marketplaces.map(mp => (
                <button key={mp.value} onClick={() => setMarketplace(mp.value)}
                  className={cn(
                    "px-3 py-1.5 rounded-md font-mono text-[10px] font-bold tracking-widest transition-all duration-200 uppercase",
                    marketplace === mp.value ? "shadow-md" : "text-muted-foreground hover:text-foreground"
                  )}
