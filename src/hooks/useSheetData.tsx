...
// ═══════════════════════════════════════════════════════════════
// ENDPOINTS — Ambos scripts tienen el mismo doGet con action=inventario|ordenes|forecast
// ═══════════════════════════════════════════════════════════════
const SHEIN_BASE  = 'https://script.google.com/macros/s/AKfycbweOy2ly59Fir1sT1lmWAaCL2oFXnxu6f2Ba9EFKHDfkYuOQVrQEG_NKFKfLgb6AqET/exec';
...
// ═══════════════════════════════════════════════════════════════
// NORMALIZAR ÓRDENES
// Cols: ID Orden, Estado, Fecha, Producto, SKU, SKU ID, Cantidad,
//       💚 LIQUIDACION, ✏️ Otros Costos, Costo Mercancía, 💰 UTILIDAD REAL, Margen %
// Solo COMPLETED/DELIVERED con liquidación > 0
// ═══════════════════════════════════════════════════════════════
function normalizeOrders(raw: any[], mp: string): SaleItem[] {
  if (!Array.isArray(raw)) return [];
  return raw.filter(r => {
    const estado = String(r['Estado'] || '').toUpperCase().trim();
    if (!r['ID Orden'] || estado === 'TOTALES') return false;
    if (!['COMPLETED', 'DELIVERED'].includes(estado)) return false;
    ...
    return {
      IDOrden:     String(r['ID Orden'] || ''),
      SKU:         String(r['SKU'] || r['SKU ID'] || ''),
...
function generateDemoInventory(mp: string): InventoryItem[] {
  return [
    ...
    { SKU:'783214469503', Producto:'Tapete Foamy Infantil 4pz 62x62', Stock:10, PrecioCompra:175, PrecioVenta:489, Categoria:'Tapetes Infantiles', PuntoReorden:5, Marketplace:mp, MargenPct:64.2, GananciaUnit:314, ValorTotal:4890, Estado:'🟢 OK' },
  ...
  for (let d = 0; d < 30; d++) {
    ...
    inventory.filter(i => i.Stock > 0).forEach(item => {
      if (Math.random() > 0.6) {
        ...
        const liq  = qty * item.PrecioVenta * 0.85; // ~15% fees
        const cost = qty * item.PrecioCompra;
        const util = liq - cost;
        sales.push({
          IDOrden: `DEMO-${d}-${item.SKU}`, SKU: item.SKU,
          Producto: item.Producto, Cantidad: qty,
          PrecioVenta: liq, Total: liq, Costo: cost,
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    let sheinInv:    InventoryItem[] = [];
    let tiktokInv:   InventoryItem[] = [];
    let sheinSales:  SaleItem[]      = [];
    let tiktokSales: SaleItem[]      = [];
    ...
    try {
      ...
      console.log('[YM] SHEIN órdenes raw:', raw.length, '→ con liquidación:', sheinSales.length);
    ...
    try {
      ...
      console.log('[YM] TikTok órdenes raw:', raw.length, '→ con liquidación:', tiktokSales.length);
    ...
    if (!anySuccess) {
      ...
      setRawSheinSales(generateDemoSales(demoS, 'SHEIN'));
      setRawTiktokSales(generateDemoSales(demoT, 'TIKTOK'));
      setUsingDemo(true);
      setLoading(false);
      return;
    }

    // Cross-fallback si un inventario falla
    ...
    // Si SHEIN no tiene órdenes aún, usar demo suave (no bloquear)
    if (sheinSales.length === 0 && sheinInv.length > 0)
      sheinSales = generateDemoSales(sheinInv, 'SHEIN');

    setRawSheinInv(sheinInv);
    setRawTiktokInv(tiktokInv);
    setRawSheinSales(sheinSales);
    setRawTiktokSales(tiktokSales);
    setLoading(false);
  }, []);
  ...
  const { inventory, sales } = (() => {
    switch (marketplace) {
      case 'SHEIN':
        return { inventory: rawSheinInv,  sales: filterByDate(rawSheinSales) };
      case 'TIKTOK':
        return { inventory: rawTiktokInv, sales: filterByDate(rawTiktokSales) };
      case 'COMBINADO':
        return {
          inventory: mergeInventory([...rawSheinInv, ...rawTiktokInv]),
          sales:     filterByDate([...rawSheinSales, ...rawTiktokSales]
            .map(s => ({ ...s, Marketplace: 'COMBINADO' }))),
        };
    }
  })();
